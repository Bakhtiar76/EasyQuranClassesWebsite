...
The supplied AI-generated references contain incidental flower variations;
do not reproduce that inconsistency literally. The live site uses one
two-level rosette family generated by `tools/graphics/gen-ornaments.mjs`:

- `rosette-reviews.svg` is the layered signature motif for major section and
  closing dividers.
- `rosette-simple.svg` is the same twelve-fold outer contour with reduced
  internal detail for compact rails, card rules and eyebrow ornaments.

Pricing sparkles/scallops and the teacher gear are distinct functional shapes,
not alternative rosette styles. The full consumer map is
`QA/GRAPHICS-PARITY.md`.
