...
- **BugDrop reporting is local-only:** Docker mounts `local/mu-plugins/eqc-bugdrop.php` as a read-only must-use plugin. It prints pinned hosted widget `v1.56.4`, targets `Bakhtiar76/EasyQuranClassesWebsite`, inherits the site's self-hosted font, and masks text-entry controls before capture. To avoid BugDrop's ~0.7–1.0s hosted installation-check delay on every open, the helper eagerly memoizes only that exact read-only GET for the current document and fails open to BugDrop's normal fetch; submission requests are untouched. The production child theme must contain no BugDrop file, loader, URL, or reference; the release-check skill and deployment rule require a zero-match `git grep` against `main` before any production push. BugDrop attachments live on its untrusted `bugdrop-screenshots` branch, which must never trigger deployment.
- **Parity rebuild loop:** `pwsh local/iterate.ps1 -Page 10-home -Route / -Out QA/after` runs the approved page builder with `--user=1`, flushes Elementor CSS, and captures five reference viewports. `-All` rebuilds every page after inline ornament regeneration. Builders re-query their freshly generated root element ID to detect silent no-op saves and require the local environment plus an editor capability.
- **CLI environment parity:** `WORDPRESS_CONFIG_EXTRA` must be present on both `wordpress` and `wpcli` services. The official Docker config evaluates it per process; defining `WP_ENVIRONMENT_TYPE=local` only on the web service leaves CLI processes reporting `production` even on the same localhost database.
- **Self-hosted typography:** DM Serif Display Latin400 and Manrope Latin variable400–700 ship with OFL notices in the child theme's `assets/fonts/`; `assets/css/fonts.css` loads them. No remote Google Fonts request is needed. Add other script subsets only when actual page text needs them.
...
- Photo replacements use tools/06-media.php via the local WP-CLI runner with --user=1. Native media synchronization preserves attachment IDs, regenerates sizes and is shared by bootstrap. Source/licence provenance lives in QA/ASSET-SOURCES.md; never reinstate old reference photo crops.
