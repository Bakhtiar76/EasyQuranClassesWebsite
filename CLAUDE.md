...
- **Novamira** (`github.com/use-novamira/novamira`, AGPL-3.0) is installed, active, and **connected via MCP** (`.mcp.json` at repo root → `novamira-localhost` server, using `@automattic/mcp-wordpress-remote`; verified end-to-end via `mcp-adapter-discover-abilities`). Local-only — never on production; DB checkpoint taken before install (`local/backups/pre-novamira-checkpoint.sql`); `release-check` asserts its absence from any release archive. The real Application Password lives only in `local/.env` (`NOVAMIRA_APP_PASSWORD`); `.mcp.json` references it as `${NOVAMIRA_APP_PASSWORD}`, which must also exist as a real Windows **User** environment variable for Claude Code's substitution to resolve it (`local/.env` is not read automatically) — see `local/README.md`. The separate `novamira` CLI's own `auth login` cannot complete from Claude Code's sandboxed execution context (`Error: The OS credential service could not complete the operation` — Windows Credential Manager isn't reachable there); run it from a normal interactive terminal if that path is ever needed. Both Bash/PowerShell tool calls referencing "novamira" and the plugin's own `wp plugin install ... --activate` install command are intermittently denied by the Claude Code auto-mode safety classifier, independent of `.claude/settings.json` allow rules — MCP tool calls are a different mechanism and are not affected.
