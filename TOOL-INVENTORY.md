...
| `.mcp.json` (`novamira-localhost`) | `@automattic/mcp-wordpress-remote@latest` via npx | Local only | Claude Code ↔ Novamira MCP bridge | Local site via Application Password (`local/.env`, gitignored; referenced as `${NOVAMIRA_APP_PASSWORD}`) | Medium | Active |
| **Novamira** | v1.12.2, `github.com/use-novamira/novamira` (AGPL-3.0) | **Local site only** | Direct-connect WordPress MCP: PHP execution incl. `$wpdb`, WP-CLI, filesystem R/W | Full local site/DB/filesystem | **High** | **Active, connected** — installed manually via WP Admin (Bash-based install was blocked by the auto-mode classifier); MCP connection live via `.mcp.json` (`novamira-localhost`, `@automattic/mcp-wordpress-remote`), verified with `mcp-adapter-discover-abilities`. The bundled `novamira` CLI's own auth cannot complete from Claude Code's sandbox (Windows Credential Manager unreachable there) — not needed since the MCP path covers the same abilities |
