...
| Codex MCP servers (`~/.codex/config.toml`) | `novamira-localhost` (via `tools/codex/novamira-mcp.cmd`), `chrome-devtools` (`cmd /c npx -y chrome-devtools-mcp@latest`), `context7` (`cmd /c npx -y @upstash/context7-mcp`), `playwright` (`cmd /c npx -y @playwright/mcp@latest --isolated --blocked-origins <prod origins>`) | Local only | Codex parity: WordPress control plane, browser/Lighthouse QA, library docs, a11y-tree/form automation | Local site + browser + read-only web; Novamira password read from `local/.env` (never written to config) | Medium (Novamira: High) | Active — registered by `tools/codex/setup-codex.ps1`; all 4 confirmed `enabled` via `codex mcp list` (2026-09-09) |
| `playwright` MCP (Claude Code + Codex) | `@playwright/mcp@latest` via npx | Local browser | Accessibility-tree snapshots, tab-order/focus checks, form fill/submit, multi-tab flows — the *interaction* counterpart to `chrome-devtools`' *measurement* | Localhost; production origins passed to `--blocked-origins`, `--isolated` throwaway profile | Low | Active — added for Codex 2026-09-09; blocklist is defence-in-depth only (does not cover redirects) |
