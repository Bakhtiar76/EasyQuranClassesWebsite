...
| WordPress (local) | `wordpress:php8.2-apache`, WP 7.1 | Local only, `http://localhost:8080` | Site under development | Local site/DB/files | Medium | Active |
| MariaDB (local) | `mariadb:11` | Local only | Local site database | Local container volume | Medium | Active |
| WP-CLI | `wordpress:cli-php8.2` (containerized) | Local only | Inspection, install, export, backups | Local WordPress/DB via `docker compose ... run --rm wpcli` or `local/wp.ps1` | Medium | Active |
...
| **Novamira** | v1.12.2, `github.com/use-novamira/novamira` (AGPL-3.0) | **Local site only** | Direct-connect WordPress MCP: PHP execution incl. `$wpdb`, WP-CLI, filesystem R/W | Full local site/DB/filesystem | **High** | Approved local-only; install pending explicit approval (see Risks) |
