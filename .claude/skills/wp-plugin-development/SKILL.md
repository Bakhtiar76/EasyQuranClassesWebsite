...
Sourced from the official [WordPress/agent-skills](https://github.com/WordPress/agent-skills) repository (`wp-plugin-development`), reviewed and adapted for this project. Applies to `wp-content/plugins/easy-quran-classes-core/` if/when it is created — see `.claude/rules/wordpress.md` and `CLAUDE.md` for this project's architecture and ownership rules, which take precedence over anything here.
...
Use `.claude/skills/wp-audit` for project-wide discovery first (already covers what a generic triage script would). Then, to deterministically scan for plugin headers already present in the repo:

```bash
node .claude/skills/wp-plugin-development/scripts/detect_plugins.mjs
```
